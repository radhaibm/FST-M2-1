-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/radha/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/radha/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/radha/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
STORE namesOrdered INTO 'hdfs:///user/radha/outputs' USING PigStorage('\t');